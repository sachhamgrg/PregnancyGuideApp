...
public class MOSterilizationFragment extends Fragment {
    ...
    Bitmap[] Bitmap_Sterilization = new Bitmap[3];    // Array of bitmap values for Sterilization images.
    Bitmap bitmap_Sterilization_overview;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.fragment_fp_mo_sterilization, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        StorageReference imageRef_S_overview = StorageFP_MedicalOptionsRef.child("Sterilization").child("SterilizationOverview.png");

        // Get a reference to the ImageView in your layout
        ImageView imgViewS_overview = getActivity().findViewById(R.id.imageViewSterilizationOverview);

        // Cache handling for the image.
        File imageFile_S_overview = new File(cacheDir, "Sterilization.png");

        // Call the method to load the image on the image view and store the returned bitmap value.
        loadImage(imageFile_S_overview, imageRef_S_overview, imgViewS_overview, bitmap -> bitmap_Sterilization_overview = bitmap);

        // Add click listener on the image if the user wishes to zoom in/out.
        view.findViewById(R.id.imageViewSterilizationOverview).setOnClickListener(v -> expandImage(bitmap_Sterilization_overview));

        // -------------------------------- Types of Barrier Method ---------------------------------

        // Toggle visibility for Types of Barrier Method Layout.
        LinearLayout TypesOfSterilizationLayout = getActivity().findViewById(R.id.layout_TypesOfSterilization);
        TextView txtViewToS = view.findViewById(R.id.txtTypesOfSterilization);
        toggleLayout(TypesOfSterilizationLayout, txtViewToS);

        // Get references to the ImageViews inside Types of Barrier Method layout.
        ImageView[] imageViewsToS = {
                getActivity().findViewById(R.id.imageViewSterilization1),
                getActivity().findViewById(R.id.imageViewSterilization2),
                getActivity().findViewById(R.id.imageViewSterilization3)
        };

        // Iterate over the ImageViews and StorageReferences, and handle the cache.
        for (int i = 0; i < imageViewsToS.length; i++) {
            String imageName = "Sterilization" + (i+1) + ".png";
            StorageReference imageRef = StorageFP_MedicalOptionsRef.child("Sterilization").child(imageName);
            ...
            loadImage(imageFile, imageRef, imageViewsToS[i], bitmap -> Bitmap_Sterilization[idx] = bitmap);

            // Add click listeners on the images if the user wishes to zoom in/out.
            imageViewsToS[i].setOnClickListener(v -> expandImage(Bitmap_Sterilization[idx]));
        }
    ...
    private void loadImage(File imageFile, StorageReference imageRef, ImageView imageView, MOSterilizationFragment.BitmapLoadCallback callback) {
