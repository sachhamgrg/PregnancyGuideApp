...
public class NutritionAdviceFragment extends Fragment {
    ...
    StorageReference StorageNutritionRef = FirebaseStorage.getInstance().getReferenceFromUrl("gs://pregnancy-guide-app-841c4.appspot.com").child("Application").child("Nutrition");
    Bitmap[] setBitmapN = new Bitmap[4];    // Array of bitmap values for Nutrition images.
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.fragment_nutrition_advice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Loading/Storing of the image file within the app.
        ...
        ImageView[] imageViews = {
                getActivity().findViewById(R.id.imageViewNutrition1),
                getActivity().findViewById(R.id.imageViewNutrition2),
                getActivity().findViewById(R.id.imageViewNutrition3),
                getActivity().findViewById(R.id.imageViewNutrition4)
        };

        // Iterate over the ImageViews and StorageReferences, and handle the cache.
        for (int i = 0; i < imageViews.length; i++) {
            String imageName = "Nutrition" + (i+1) + ".png";
            StorageReference imageRef = StorageNutritionRef.child(imageName);
            ...
            int idx = i;
            loadImage(imageFile, imageRef, imageViews[i], bitmap -> setBitmapN[idx] = bitmap);
        }

        // Ensures the image is expanded once the transition is finished on each imageViews.
        MotionLayout motionLayout = view.findViewById(R.id.motionLayoutNutrition);
        motionLayout.setTransitionListener(new MotionLayout.TransitionListener() {
            ...
            public void onTransitionCompleted(MotionLayout motionLayout, int currentId) {
                ...
                switch (currentId) {
                    case R.id.endN1:
                        expandImage(setBitmapN[0],motionLayout);
                        break;
                    case R.id.endN2:
                        expandImage(setBitmapN[1],motionLayout);
                        break;
                    case R.id.endN3:
                        expandImage(setBitmapN[2],motionLayout);
                        break;
                    case R.id.endN4:
                        expandImage(setBitmapN[3],motionLayout);
                        break;
                }
            }
            @Override
    ...
    private void loadImage(File imageFile, StorageReference imageRef, ImageView imageView, NutritionAdviceFragment.BitmapLoadCallback callback) {

        if (imageFile.exists()) {
            // Image already exists in the cache directory, load it from the cache.
